...
-module(request_location_server).
...
get_location_of(PackageID)-> gen_server:call(?MODULE, {package_loc,PackageID,Location}).
...
init([]) ->
	%{Success, Riak_PID} = riakc_pb_socket:start_link("rdb.fordark.org", 8087).
    	case riakc_pb_socket:start_link("URL.com", 8087) of 
...
%% @end
%%--------------------------------------------------------------------
handle_call({package_loc,PackageID}, _From, Riak_PID) ->
    	%{reply,<<location>>,Riak_PID};
	case riakc_pb_socket:get(Riak_PID, <<"location">>, PackageID) of 
	    {ok,Fetched}->
